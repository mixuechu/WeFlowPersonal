...
const RECOVERY_PAYLOAD_CODEC = 'gzip-json-v1'
...
export class PersonalMemoryStore {
  ...
  initialize(databasePath: string, encryptionKey?: Buffer | string): void {
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS assistant_answer_dependencies (
        ...
        evidence_scope_restricted INTEGER NOT NULL DEFAULT 0,
        created_at TEXT NOT NULL,
    ...
    this.ensureColumn('assistant_answer_dependencies', 'evidence_scope_restricted', 'INTEGER NOT NULL DEFAULT 0')
    this.db.prepare(`
  ...
  private ensureMemorySearchFeedbackArchiveRevisionTriggers(): void {
    this.ensureRevisionTriggerSet({
      prefix: 'memory_search_feedback_archive_revision',
      revisionKey: 'memory_search_feedback_archive_revision',
      tables: ['memory_search_feedback'],
      version: 'memory-search-feedback-archive-revision-v2',
      nameFor: (_table, operation) =>
        `trg_memory_search_feedback_archive_revision_${operation.toLowerCase()}`
    })
  }
  ...
  private ensureMemoryDeletionAuditRevisionTriggers(): void {
    this.ensureRevisionTriggerSet({
      prefix: 'memory_deletion_audit_revision',
      revisionKey: 'memory_deletion_audit_revision',
      tables: ['memory_deletion_audit'],
      version: 'memory-deletion-audit-revision-v2',
      nameFor: (_table, operation) =>
  ...
  private revisionTriggerDefinitions(
    prefix: string,
    tables: string[],
    whenFor?: (
      table: string,
    ...
    return tables.flatMap(table =>
      (['INSERT', 'UPDATE', 'DELETE'] as const).map(operation => ({
        name: nameFor?.(table, operation) ||
          `trg_${prefix}_${table}_${operation.toLowerCase()}`,
        table,
        operation,
        when: String(whenFor?.(table, operation) || '').trim()
      })))
  }
  ...
  private inspectRevisionTriggerSet(
    prefix: string,
    revisionKey: string,
    tables: string[],
    whenFor?: (
    ...
    const definitions = this.revisionTriggerDefinitions(
      prefix, tables, whenFor, nameFor
    )
    if (!this.db) {
      return {
        expectedTriggers: definitions.length,
  ...
  private ensureRevisionTriggerSet(input: {
    ...
    const before = this.inspectRevisionTriggerSet(
      input.prefix, input.revisionKey, input.tables, input.whenFor, input.nameFor
    )
    ...
    try { previous = JSON.parse(String(previousRow?.value || '{}')) } catch {}
    const definitions = this.revisionTriggerDefinitions(
      input.prefix, input.tables, input.whenFor, input.nameFor
    )
    const definitionByName = new Map(definitions.map(item => [item.name, item]))
    const quoteIdentifier = (value: string) => `"${value.replace(/"/g, '""')}"`
    if (!before.healthy) {
  ...
  private resourceArchiveRevisionTables(): string[] {
    return ['memory_resources', 'search_document_evidence', 'resource_trash']
  }

  private resourceArchiveRevisionWhen(
    table: string,
    operation: 'INSERT' | 'UPDATE' | 'DELETE'
  ): string {
    if (table !== 'search_document_evidence') return ''
    if (operation === 'INSERT') return `WHEN NEW.document_id LIKE 'resource:%'`
    if (operation === 'DELETE') return `WHEN OLD.document_id LIKE 'resource:%'`
    return `WHEN OLD.document_id LIKE 'resource:%' OR NEW.document_id LIKE 'resource:%'`
  }

  private ensureResourceArchiveRevisionTriggers(): void {
    this.ensureRevisionTriggerSet({
      prefix: 'resource_archive_revision',
      revisionKey: 'resource_archive_revision',
      tables: this.resourceArchiveRevisionTables(),
      version: 'resource-archive-revision-v2',
      whenFor: (table, operation) =>
        this.resourceArchiveRevisionWhen(table, operation)
    })
  }
  ...
  private ensureIdentityMergeArchiveRevisionTriggers(): void {
    this.ensureRevisionTriggerSet({
      prefix: 'identity_merge_archive_revision',
      revisionKey: 'identity_merge_archive_revision',
      tables: ['merge_history'],
      version: 'identity-merge-archive-revision-v2'
    })
  }
  ...
  private ingestionArchiveRevisionTables(): string[] {
    return ['ingestion_runs', 'ingestion_batches']
  }

  private ensureIngestionArchiveRevisionTriggers(): void {
    this.ensureRevisionTriggerSet({
      prefix: 'ingestion_archive_revision',
      revisionKey: 'ingestion_archive_revision',
      tables: this.ingestionArchiveRevisionTables(),
      version: 'ingestion-archive-revision-v2'
    })
  }
  ...
  private ensureIngestionRecoveryRevisionTriggers(): void {
    this.ensureRevisionTriggerSet({
      prefix: 'ingestion_recovery_revision',
      revisionKey: 'ingestion_recovery_revision',
  ...
  private repairStructuredSearchIndex(options: { dryRun?: boolean } = {}): any {
    ...
    const queueStructuredDocumentRepair = (
      ...
      try { current = JSON.parse(String(document.metadata_json || '{}')) } catch {
        parseFailed = true
      }
      const next = { ...current, ...authoritative }
    ...
    this.db.transaction(() => {
      ...
      const audit = {
        ...
        metadataDocumentsRepairedTotal: Math.max(
          0,
  ...
  private compactFailedCrossStoreMutationPayloads(): void {
    if (!this.db) return
    const taskRows = this.db.prepare(`
      SELECT commit_id,before_tokens_json,after_tokens_json,changes_json
      FROM task_mutation_commits
      WHERE status='prepared' AND recovery_attempts>0 AND payload_codec=''
    `).all() as any[]
    const sourceRows = this.db.prepare(`
      SELECT commit_id,before_tokens_json,after_tokens_json,policies_json
    ...
    this.db.transaction(() => {
      ...
      for (const row of taskRows) {
        const compressed = compressRecoveryPayload({
          before_tokens_json: String(row.before_tokens_json || '{}'),
          after_tokens_json: String(row.after_tokens_json || '{}'),
  ...
  listPreparedTaskMutationCommits(limit = 100): Array<{
    ...
    `).all(Math.max(1, Math.min(500, Math.floor(limit || 100)))) as any[]).map(row => {
      ...
      try {
        const parsed = JSON.parse(String(payload.before_tokens_json || '{}'))
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error()
        beforeTokens = parsed
      } catch { failures.push('变更前身份') }
      try {
        const parsed = JSON.parse(String(payload.after_tokens_json || '{}'))
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) throw new Error()
        afterTokens = parsed
      } catch { failures.push('变更后身份') }
      try {
        const parsed = JSON.parse(String(payload.changes_json || '[]'))
  ...
  finalizeTaskMutationCommit(commitId: string, tasks: any[]): void {
    ...
    this.db.transaction(() => {
      const row = this.db!.prepare(`
        SELECT * FROM task_mutation_commits WHERE commit_id=?
  ...
  recordTaskMutationRecoveryFailure(commitId: string, error: unknown): void {
    if (!this.db) return
    this.db.transaction(() => {
      this.compactTaskMutationPayload(commitId)
      this.db!.prepare(`
        UPDATE task_mutation_commits
  ...
  correctEvent(id: string, input: {
    ...
    const participants = replacesParticipants
      ? [...new Map(input.participants!.map(item => {
          ...
          const role = String(item.role || 'participant').trim().slice(0, 120) || 'participant'
          if (!entityId) throw new Error('事件参与者不能为空')
          const entity = participantEntity.get(entityId) as { canonical_name?: string } | undefined
          if (!entity) throw new Error('事件参与者不存在或已经删除')
      ...
      : existingParticipants.items
    const now = new Date().toISOString()
    const beforeSnapshot = {
      ...before,
      participants: existingParticipants.items
    }
    const after = {
      ...before,
      ...
      search_text: [
        ...
        ...participants.flatMap((participant: any) =>
          [participant.canonical_name, participant.role])
      ].filter(Boolean).join('；'),
      updated_at: now
    }
    const transaction = this.db.transaction(() => {
      ...
      this.db!.prepare(`
        ...
        VALUES('event',?,?,?,?)
      `).run(id, JSON.stringify(beforeSnapshot), JSON.stringify(after), now)
      const indexedParticipants = this.db!.prepare(`
        SELECT ep.entity_id,e.canonical_name,ep.role
        FROM event_participants ep LEFT JOIN entities e ON e.id=ep.entity_id
        WHERE ep.event_id=?
      `).all(id) as any[]
      this.upsertSearchDocument(`event:${id}`, 'event', id, after.title, after.search_text, {
        eventType: after.event_type,
        startAt: after.start_at,
        endAt: after.end_at,
        participantIds: indexedParticipants.map(participant => participant.entity_id),
        status: 'confirmed',
        sourceNature: 'human_confirmation',
        correctionCount: Number((this.db!.prepare(`
  ...
  saveEmbedding(
    ...
    if (!validateEmbeddingBatch(normalizedChunks.map(item => item.vector), normalizedChunks.length).valid
      || normalizedChunks.some(item => item.chunkHash === undefined)) return false
    return this.db.transaction(() => {
  ...
  private replaceEmbeddingChunks(
    ...
    if (!this.db) return
    const document = this.db.prepare(`
      SELECT title,search_text FROM search_documents WHERE id=?
    `).get(documentId) as any
    ...
    chunks.forEach((chunk, index) => {
      const fallback = chunks.length === 1
      const startOffset = Number.isInteger(chunk.startOffset)
        ? Number(chunk.startOffset)
        : fallback ? 0 : -1
      const endOffset = Number.isInteger(chunk.endOffset)
        ? Number(chunk.endOffset)
        : fallback ? authoritativeText.length : -1
      if (startOffset < 0 || endOffset <= startOffset || endOffset > authoritativeText.length) {
        throw new Error('向量块位置超出当前权威正文，未写入该批')
      }
      const authoritativeChunk = authoritativeText.slice(startOffset, endOffset)
      const chunkHash = createHash('sha256').update(authoritativeChunk).digest('hex')
      if (chunk.chunkHash && chunk.chunkHash !== chunkHash) {
        throw new Error('向量块正文指纹与当前权威文档不一致，未写入该批')
      }
      insert.run(
        documentId,
        index,
  ...
  private rankVectorRows(rows: any[], vector: number[], limit: number, mode: 'exact' | 'ann'): any[] {
    const ranked = rows.map(row => {
      ...
      const authoritativeText = `${String(row.title || '')}\n${String(row.search_text || '')}`
        .replace(/\r\n?/g, '\n').trim()
      const startOffset = Number(row.start_offset)
      const endOffset = Number(row.end_offset)
      const semanticMatchExcerpt = Number.isInteger(startOffset)
        && Number.isInteger(endOffset)
        && startOffset >= 0
        && endOffset > startOffset
        && endOffset <= authoritativeText.length
        ? authoritativeText.slice(startOffset, endOffset)
        : ''
      return {
        ...publicRow,
        semantic_score: score,
        semantic_search_mode: mode,
        semantic_match_excerpt: semanticMatchExcerpt,
        semantic_match_chunk_index: Number.isInteger(Number(row.chunk_index))
  ...
  searchVector(
    ...
    for (let offset = 0; offset < ids.length; offset += 500) {
      ...
      rows.push(...this.db.prepare(`
        SELECT d.*,embedding.vector_json AS embedding_json,embedding.chunk_index,
          embedding.start_offset,embedding.end_offset
        FROM search_documents d
        JOIN search_document_embedding_chunks embedding ON embedding.document_id=d.id
          AND embedding.model=d.embedding_model AND embedding.content_hash=d.content_hash
  ...
  private repairAssistantAnswerDependencies(): void {
    if (!this.db) return
    const key = 'assistant_answer_dependencies_v4'
    const existing = this.db.prepare('SELECT value FROM schema_meta WHERE key=?').get(key) as any
    ...
    const insert = this.db.prepare(`
      INSERT INTO assistant_answer_dependencies(
        ...
        evidence_authority_revision,evidence_scope_restricted,created_at
      ) VALUES(?,?,?,?,?,?,?,?,?,?,?)
      ON CONFLICT(message_id,statement_index,document_id) DO UPDATE SET
        ...
        evidence_scope_restricted=excluded.evidence_scope_restricted
    `)
        ...
        statements.forEach((rawIds: any, statementIndex: number) => {
          ...
          for (const documentId of documentIds) {
            ...
            indexedDependencies += insert.run(
              ...
              citation?.evidenceScopeRestricted ? 1 : 0,
              row.created_at
            ).changes
          }
        })
  ...
  getAssistantAnswerDependencyStats(): any {
    if (!this.db) return {
      version: 4, messages: 0, statements: 0, dependencies: 0,
    ...
    `).get() as any
    const row = this.db.prepare(`
      SELECT value FROM schema_meta WHERE key='assistant_answer_dependencies_v4'
    `).get() as any
    let audit: any = {}
    try { audit = JSON.parse(String(row?.value || '{}')) } catch {}
    return {
  ...
  listAssistantConversationsPage(options: {
    ...
    const evidenceCountsCurrent = `(d.evidence_scope_restricted=1
      OR d.evidence_sample_hash=''
    ...
    const revalidationCte = `
      ...
      dependency_state AS (
        SELECT d.conversation_id,d.message_id,d.statement_index,
          ...
          CASE WHEN ${eligibility} AND (
            d.content_hash=''
            OR (
              d.evidence_scope_restricted=1
              AND d.content_hash!=''
              AND lower(d.content_hash)=lower(s.content_hash)
              AND NOT ${evidenceAuthorityCurrent}
            )
          ...
          ,CASE WHEN ${eligibility} AND d.content_hash!=''
            AND lower(d.content_hash)=lower(s.content_hash)
            AND ${evidenceCountsCurrent}
            AND d.evidence_scope_restricted=0
            AND NOT ${evidenceAuthorityCurrent} THEN 1 ELSE 0 END AS is_evidence_authority_changed,
          CASE WHEN ${eligibility} AND d.evidence_scope_restricted=1
            AND d.content_hash!='' AND lower(d.content_hash)=lower(s.content_hash)
            AND NOT ${evidenceAuthorityCurrent}
            THEN 1 ELSE 0 END AS is_scoped_evidence_changed
      ...
      statement_state AS (
        SELECT conversation_id,message_id,statement_index,
          ...
          MAX(is_scoped_evidence_changed) AS has_scoped_evidence_changed
      ...
      answer_revalidation AS (
        SELECT ss.conversation_id,ss.message_id,m.created_at,
          ...
          SUM(CASE WHEN ss.has_current=0 AND ss.has_unknown=1
            AND ss.has_scoped_evidence_changed=1
            THEN 1 ELSE 0 END) AS scoped_evidence_review_statements,
      ...
      conversation_revalidation AS (
        SELECT conversation_id,
          ...
          SUM(scoped_evidence_review_statements) AS scoped_evidence_review_statements,
    ...
    const items = this.db.prepare(`
      ...
      SELECT c.id,c.title,c.created_at,c.updated_at,COUNT(m.id) AS message_count,
        ...
        COALESCE(cr.scoped_evidence_review_statements,0)
          AS revalidation_scoped_evidence_review_statements,
  ...
  listAssistantAnswerReviewsPage(options: {
    ...
    const revalidationCte = `
      ...
      statement_state AS (
        SELECT d.conversation_id,d.message_id,d.statement_index,
          ...
          MAX(d.is_scoped_evidence_changed) AS has_scoped_evidence_changed,
          weflow_sha256((
    ...
    const items = this.db.prepare(`
      ...
      SELECT ar.message_id,ar.conversation_id,ar.created_at,ar.state_key,
        ...
        ar.scoped_evidence_review_statements,
        ar.missing_statements,ar.ineligible_statements,ar.content_changed_statements,
        ar.evidence_counts_changed_statements,ar.evidence_changed_statements,
  ...
  finalizeConversationSourceMutationCommit(commitId: string): void {
    ...
    this.db.transaction(() => {
      const row = this.db!.prepare(`
        SELECT * FROM conversation_source_mutation_commits WHERE commit_id=?
      `).get(String(commitId || '')) as any
  ...
  recordConversationSourceMutationRecoveryFailure(commitId: string, error: unknown): void {
    if (!this.db) return
    this.db.transaction(() => {
      this.compactConversationSourceMutationPayload(commitId)
      this.db!.prepare(`
        UPDATE conversation_source_mutation_commits
        SET recovery_attempts=recovery_attempts+1,last_error=?
        WHERE commit_id=? AND status='prepared'
