...
export async function extractScannedPdfText(filePath: string): Promise<{
  success: boolean
  text: string
  ...
  if (!status.available) {
    return { success: false, text: '', status: 'dependency_missing', processedPages: 0, totalPages: 0, truncated: false }
  }
  ...
  try {
    ...
    await execFileAsync(executable('pdftoppm'), [
      '-f', '1', '-l', String(MAX_PAGES), '-r', String(RENDER_DPI), '-png', filePath, prefix
    ...
    for (const [index, page] of pages.entries()) {
      ...
      if (result.success && meaningfulCharacters.length >= 2) text.push(`[第 ${index + 1} 页] ${recognized}`)
    }
