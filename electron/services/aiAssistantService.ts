...
export class AiAssistantService {
  ...
  private taskLifecycleAuditPromise: Promise<any> | null = null
  private taskLifecycleAuditState = {
    running: false, total: 0, processed: 0, closed: 0, kept: 0, skipped: 0,
    startedAt: '', finishedAt: '', lastError: ''
  }
  ...
  private loadState(): void {
    try {
      ...
      this.state = {
        ...
        taskLifecycleAudit: {
          ...structuredClone(EMPTY_STATE.taskLifecycleAudit),
          ...(loaded.taskLifecycleAudit || {}),
          running: false,
...
请输出 json。`, redactionLevel)
  ...
  auditActiveTaskLifecycles(): Promise<any> {
    ...
    const promise = this.runActiveTaskLifecycleAudit(candidateIds, startOffset, apiKey)
      ...
      .finally(() => {
        if (this.taskLifecycleAuditPromise === promise) this.taskLifecycleAuditPromise = null
      })
    this.taskLifecycleAuditPromise = promise
    return promise
  }
  ...
  private async runActiveTaskLifecycleAudit(
    ...
    for (let offset = startOffset; offset < candidateIds.length; offset += 10) {
      ...
      const mutationTokens = new Map(batch.map(task => [task.id, buildTaskMutationToken(task)]))
      const auditInput = batch.map(task => ({
        taskId: task.id,
        title: task.title,
        detail: task.detail,
        status: task.status,
        due: task.due,
        source: task.source,
        evidence: (evidenceWindowByTask.get(task.id) || []).map((item, index) => ({
          evidenceId: `${task.id}:e${index + 1}`,
          time: Number(item.timestamp || 0)
            ? new Date(Number(item.timestamp) * 1000).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })
            : '',
          sender: item.sender,
  ...
  getProjectWorkspace(projectId: string): any {
    ...
    if (!projectEntity && !derivedProject) throw new Error('项目不存在或已经不在当前可信视图中')
    const memoryFeed = projectEntity
      ? personalMemoryStore.getEntityMemory(id, 1, true)
      : { claims: [], events: [] }
    const project = projectEntity ? buildProjectInsight({
      entities: [projectEntity], relations: [], claims: [], events: [], tasks: []
    }, id) : {
      id, entityId: null, name: derivedProject!.name, summary: '', inferred: true,
      members: [], tasks: [], risks: [], milestones: [], decisions: [], claims: [],
      pendingReview: { relations: [], claims: [], milestones: [], decisions: [], total: 0 },
    ...
    const projectEvidence = boundedEvidencePayload(
    ...
    return {
      ...
      payloadPolicy: {
        ...
        eventLimit: 0,
        loadedOnDemand: true,
        taskDirectory: projectEntity ? 'sqlcipher_paginated_40' : 'sqlcipher_explicit_project_paginated_40',
        riskDirectory: projectEntity ? 'sqlcipher_union_paginated_40' : 'sqlcipher_explicit_project_union_paginated_40',
  ...
  getProjectTaskPage(projectId: string, options: any = {}): any {
    ...
    const names = projectEntity
      ? personalMemoryStore.listGraphEntityTaskSearchNames(id, { projectOnly: true })
      : [derivedProject!.normalizedName]
    const page = personalMemoryStore.listProjectTaskPage(names, {
  ...
  private applyTaskPatch(task: AssistantTask, patch: any, updatedAt: string): AssistantTask {
    ...
    if (typeof patch.lifecycleEvidence === 'string') {
      next.lifecycleEvidence = redactLocalSecrets(patch.lifecycleEvidence).trim().slice(0, 300)
    }
