...
test('memory growth is a first-class pageable archive with current dossier navigation', () => {
  ...
  assert.match(backupMethod, /isRestorable: backup => isJointMemoryBackupRestorable/)
  assert.match(backupMethod, /inspectDatabase: path => personalMemoryStore\.inspectBackup\(path\)/)
  assert.match(backupMethod, /recoverySource: state\.recovery\.source/)
