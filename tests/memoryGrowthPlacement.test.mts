...
test('memory growth is a first-class pageable archive with current dossier navigation', () => {
  ...
  assert.match(page, /disabled=\{memoryMaintenanceBusy \|\| !availability\.enabled\}/)
  ...
  assert.match(service, /stageMemoryBackupTrash/)
  assert.match(service, /shell\.trashItem\(stagingDirectory\)/)
  assert.match(service, /recoverInterruptedMemoryBackupTrash/)
  assert.match(page, /输入“移到废纸篓”确认/)
  assert.match(page, /previewDeleteMemoryBackup/)
  assert.match(page, /deleteMemoryBackup/)
})
