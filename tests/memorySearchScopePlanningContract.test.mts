...
test('memory search facets stay in SQLCipher instead of materializing one id set per facet', () => {
  ...
  assert.match(page, /if \(offset === 0\) \{[\s\S]*countSearchDocumentsInScope/)
  assert.match(page, /if \(searchFacets\) Object\.assign\(page, searchFacets\)/)
  const loadMoreStart = pageSource.indexOf('const loadMoreMemoryResults')
  const loadMoreEnd = pageSource.indexOf('const memoryFeedbackSavingAction', loadMoreStart)
  const loadMore = pageSource.slice(loadMoreStart, loadMoreEnd)
  assert.match(loadMore, /setMemorySearchState\(current => \(\{[\s\S]*\.\.\.current/)
  assert.doesNotMatch(loadMore, /typeCounts: page\.typeCounts|reviewPresetCounts: page\.reviewPresetCounts/)
})
...
test('scope planning is honest and visible in complete diagnostics', () => {
  ...
  assert.match(store, /relationDossierLookup: 'sqlcipher_point_by_stable_id'[\s\S]*?relationDossierEvidenceLimit: MEMORY_CARD_EVIDENCE_LIMIT[\s\S]*?relationDossierFullGraphMaterializations: 0[\s\S]*?relationDossierEndpointHydration: 'requested_ids_only'/)
  assert.match(pageSource, /relationDossierLookup === 'sqlcipher_point_by_stable_id' \? 'SQLCipher 稳定 ID 点查'/)
  assert.match(pageSource, /关系原文预览 <b>\{Number\(memoryDiagnostics\.memorySearchScopePlanning\.relationDossierEvidenceLimit/)
  assert.match(pageSource, /关系档案全图副本 <b>\{Number\(memoryDiagnostics\.memorySearchScopePlanning\.relationDossierFullGraphMaterializations/)
  assert.match(pageSource, /relationDossierEndpointHydration === 'requested_ids_only' \? '仅所需实体'/)
...
test('graph viewport selection and relation budgets stay in SQLCipher', () => {
  ...
  assert.match(workspace, /listEntityRelatedTaskPage\([\s\S]*?listGraphEntityTaskSearchNames/)
  ...
  assert.match(taskPage, /getGraphEntityById\([\s\S]*?listGraphEntityTaskSearchNames/)
