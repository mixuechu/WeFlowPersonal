...
test('scope planning is honest and visible in complete diagnostics', () => {
  ...
  assert.match(pageSource, /可信实体目录 <b>[\s\S]*?SQLCipher 搜索分页[\s\S]*?实体目录 ID 集合[\s\S]*?同名碰撞[\s\S]*?全可信范围计数[\s\S]*?实体选择[\s\S]*?revision 保护/)
