...
test('scope planning is honest and visible in complete diagnostics', () => {
  assert.match(store, /memorySearchScopePlanning:\s*\{[\s\S]*?version: 6[\s\S]*?facetStrategy: 'sqlcipher_direct_count'[\s\S]*?facetIdentityMaterializations: 0[\s\S]*?primaryScopeStrategy: 'sqlcipher_isolated_temp_table'[\s\S]*?primaryIdentityMaterializations: 0[\s\S]*?hybridScopeReused: true[\s\S]*?releasedAfterRequest: true[\s\S]*?handleOnlyScopeApi: true[\s\S]*?legacySharedScopeRemoved: true[\s\S]*?scopedGraphPathStrategy: 'sqlcipher_recursive_cte'[\s\S]*?scopedGraphRelationIdentityMaterializations: 0[\s\S]*?scopedGraphPathExpansionBudget: SCOPED_GRAPH_PATH_EXPANSION_LIMIT[\s\S]*?scopedGraphPathBreadthFirst: true[\s\S]*?scopedGraphPathTruncationVisible: true[\s\S]*?unscopedGraphPathStrategy: 'sqlcipher_recursive_cte'[\s\S]*?unscopedGraphAdjacencyMaterializations: 0/)
