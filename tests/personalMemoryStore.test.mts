...
test('relation graph snapshots keep a bounded hotset while SQLCipher retains every evidence row', () => {
  ...
  try {
    ...
    assert.equal(snapshot.relations[0].evidence.length, 0)
    assert.equal(snapshot.relations[0].evidenceTotal, allEvidence.length)
    const hotset = first.getRelationEvidenceHotset(
      ['hotset-relation'], GRAPH_RELATION_EVIDENCE_HOT_LIMIT
    ).get('hotset-relation')!
    assert.equal(hotset.evidence.length, GRAPH_RELATION_EVIDENCE_HOT_LIMIT)
    assert.equal(hotset.evidenceTotal, allEvidence.length)
    ...
    assert.equal(hydration.pendingReviews, 0)
    ...
    assert.ok(hydration.lastLoadedAt)
  ...
  try {
    ...
    assert.equal(snapshot.relations[0].evidence.length, 0)
    assert.equal(snapshot.relations[0].evidenceTotal, allEvidence.length)
    assert.equal(reopened.getRelationEvidenceHotset(['hotset-relation'], 8)
      .get('hotset-relation')?.evidence.length, 8)
  } finally {
    reopened.close()
    rmSync(directory, { recursive: true, force: true })
  }
...
test('batched graph hydration counts identity evidence across an active merge chain without resident keys', () => withStore(store => {
  store.syncGraph({
    entities: [{
      ...
      trustStatus: 'confirmed', evidenceMessageIds: ['wechat:merge-room:source-evidence']
    }, {
      id: 'hydration-merge-target', type: 'person', canonicalName: '合并后身份',
      trustStatus: 'confirmed', evidenceMessageIds: ['wechat:merge-room:target-evidence']
    }],
    relations: [],
    reviewQueue: []
  } as any, 'hydration-merge-before')
  const database = (store as any).db
  database.prepare(`
    INSERT INTO entity_evidence(
      entity_id,source_id,message_id,session_id,timestamp,sender,excerpt,evidence_kind
  ...
  `).run(
    'hydration-merge-source', 'hydration-merge-target', '合并前身份', '合并后身份', '{}',
    '2026-08-09T08:00:00.000Z'
  )
  database.prepare(`UPDATE entities SET deleted_at=? WHERE id=?`)
    .run('2026-08-09T08:00:00.000Z', 'hydration-merge-source')
...
test('batched graph hydration keeps query count fixed at multi-year graph scale', () => withStore(store => {
  ...
  const relationCount = 3_000
  const entities = Array.from({ length: entityCount }, (_, index) => ({
    id: `hydration-scale-entity-${index}`,
    type: index % 5 === 0 ? 'project' : 'person',
    canonicalName: `规模实体 ${index}`,
    aliases: [`规模别名 ${index}`],
    accountIds: index % 2 === 0 ? [`wxid-hydration-scale-${index}`] : [],
    trustStatus: 'confirmed'
  }))
  const relations = Array.from({ length: relationCount }, (_, index) => ({
    id: `hydration-scale-relation-${index}`,
    subjectId: entities[index % entityCount].id,
    predicate: '关联',
    objectId: entities[(index * 17 + 1) % entityCount].id,
    confidence: 0.8,
    status: 'confirmed',
    evidence: [{
      sourceId: 'wechat',
      sessionId: `hydration-scale-room-${index % 20}`,
      messageId: `hydration-scale-message-${index}`,
      timestamp: 1_700_000_000 + index,
      sender: `规模实体 ${index % entityCount}`,
      excerpt: `规模关系原文 ${index}`
    }]
  }))
  store.syncGraph({ entities, relations, reviewQueue: [] } as any, 'hydration-scale')

  const snapshot = store.loadGraphSnapshot()
  const hydration = store.getGraphSnapshotHydrationStats()
  assert.equal(snapshot.entities.length, entityCount)
  assert.equal(snapshot.relations.length, relationCount)
  assert.equal(hydration.queryCount, 8)
  assert.equal(hydration.entities, entityCount)
  assert.equal(hydration.relations, relationCount)
  ...
  assert.ok(['healthy', 'attention', 'critical'].includes(hydration.performanceStatus))
  assert.ok(hydration.durationMs < 5_000,
    `2,000 entities and 3,000 relations hydrated in ${hydration.durationMs}ms`)
}))
...
test('graph commit mismatch recovers authoritative entities relations evidence and pending reviews', () => {
  withStore(store => {
    ...
    assert.equal(snapshot.relations[0].evidence.length, 0)
    assert.equal(snapshot.relations[0].evidenceTotal, 1)
    assert.equal(store.getRelationEvidenceHotset(['graph-recovery-relation'])
      .get('graph-recovery-relation')?.evidence[0].sender, '恢复甲')
    assert.equal(snapshot.reviewQueue.length, 1)
    ...
    assert.equal(hydration.entities, 2)
    assert.equal(hydration.relations, 1)
    assert.equal(hydration.pendingReviews, 1)
    assert.equal(hydration.aliasRows, 1)
    assert.equal(hydration.identityRows, 2)
