...
test('power state changes gate only automatic vector continuation and resume it on AC', () => {
  ...
  assert.match(main, /try \{[\s\S]*process\.getSystemMemoryInfo\(\)[\s\S]*aiAssistantService\.updatePowerState/)
  assert.match(main, /catch \(error\) \{[\s\S]*try \{[\s\S]*reportPowerStateMeasurementFailure\(error\)[\s\S]*catch \(reportError\)/)
...
test('power deferral is visible beside semantic backlog progress', () => {
  ...
  assert.match(page, /资源测量异常/)
  assert.match(page, /当前沿用上次安全策略/)
  assert.match(service, /reportPowerStateMeasurementFailure\(error: unknown\)/)
  assert.match(service, /lastMeasurementError: sanitizeDiagnosticText\(error\)/)
  assert.match(service, /lastMeasurementErrorAt: ''[\s\S]*lastMeasurementError: ''/)
})
