        ...
        <div className="assistant-modal-backdrop">
          <div className="assistant-diagnostics-modal">
            ...
            {memoryDiagnostics.graphEntityEvidenceHotset?.version && <div className="assistant-recovery-audit healthy">
              <header><Database size={15} /><span><b>图谱身份原文分层</b>
                <small>人物与项目的完整身份原文及有效合并链由 SQLCipher 权威保存；启动不常驻历史消息键，当前尚未提交的新证据与合并操作仍使用有界临时窗口。</small>
              </span></header>
              <div className="assistant-recovery-current">
            ...
            {memoryDiagnostics.graphReviewEvidenceStorage?.version && <div className={`assistant-recovery-audit ${memoryDiagnostics.graphReviewEvidenceStorageHealthy ? 'healthy' : 'unhealthy'}`}>
              ...
              <div className="assistant-recovery-current">
                ...
                <span>本次延后复核 <b>{Number(memoryDiagnostics.graphReviewEvidenceStorage.provenanceRepair?.deferredRowsThisStart || 0).toLocaleString()}</b> 条{memoryDiagnostics.graphReviewEvidenceStorage.provenanceRepair?.nextRetryAt
                  ? ` · 下次 ${new Date(memoryDiagnostics.graphReviewEvidenceStorage.provenanceRepair.nextRetryAt).toLocaleString('zh-CN', { hour12: false })}` : ''}</span>
                <span>本次有界批次/权威联查 <b>{Number(memoryDiagnostics.graphReviewEvidenceStorage.provenanceRepair?.batchesThisStart || 0).toLocaleString()} / {Number(memoryDiagnostics.graphReviewEvidenceStorage.provenanceRepair?.authorityQueriesThisStart || 0).toLocaleString()}</b></span>
                <span>单批峰值 <b>{Number(memoryDiagnostics.graphReviewEvidenceStorage.provenanceRepair?.peakRowsThisStart || 0).toLocaleString()}</b> 行 / {Number(memoryDiagnostics.graphReviewEvidenceStorage.provenanceRepair?.peakCarriersThisStart || 0).toLocaleString()} 个载体（上限 {Number(memoryDiagnostics.graphReviewEvidenceStorage.provenanceRepair?.batchLimit || 0).toLocaleString()}）</span>
